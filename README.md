...
- 🧠 **ReAct Agent 决策循环**：把检索注册为工具（Function Calling），模型自主决定何时检索、检索几次（跨文档对比题会自动拆成多次检索）、要不要检索——而不是代码写死流程
- 💬 **多轮对话**：自动把"它的范围？"这类指代词问题改写为独立问题再检索
- 👋 **闲聊识别**：「你是谁」这类寒暄直接友好回复，不浪费检索和模型调用
- 👆 **文档片段预览**：点击左侧文档卡片即可展开查看切分后的片段内容
...
**Q：这是真正的 Agent 还是固定流程的 RAG？**
是 ReAct 式 Agent：把 `search_knowledge_base`、`list_documents` 注册为工具（OpenAI Function Calling 格式），模型在每轮自主决策——该不该检索、用什么关键词、检索几次、还是直接回答。实测"年假和云盘回收站保留时间分别是多久？"这类跨文档对比题，模型会自动拆成两次检索再合并作答；闲聊类问题则不检索直接回复。保留固定流水线作为降级路径（模型不支持工具调用时自动切换），并对循环设了最大轮数上限防失控。
...
> - 实现 ReAct 式 Agent 决策循环：将混合检索注册为工具（Function Calling），由模型自主决策检索时机与次数，支持多跳检索与工具调用降级；
